...
export const getCurrentUser = async (jwt: string) => {
  try {
    ...
    const res = await axios.get("https://e-learning-smbe.onrender.com/api/users/me", {
      headers: {
    ...
    try {
      ...
      const userWithRoleRes = await axios.get(`https://e-learning-smbe.onrender.com/api/users/${userData.id}?populate=role`, {
        headers: {
          Authorization: `Bearer ${jwt}`,
        },
      });
    ...
    try {
      ...
      // Get all roles
      const rolesRes = await axios.get("https://e-learning-smbe.onrender.com/api/users-permissions/roles");
