...
const CourseDetailPage = () => {
  ...
  useEffect(() => {
    if (
      userAuth?.id &&
      course?.user_courses?.some(
        (uc) => Number(uc.userId) === Number(userAuth.id)
      )
    ) {
      setIsEnrolled(true);
    } else {
      ...
      const handlePaymentReturn = async () => {
        ...
        if (vnp_ResponseCode === "00" && vnp_TransactionStatus === "00") {
          try {
            ...
            const pendingCourseId = localStorage.getItem(
              "pendingEnrollmentCourseId"
            );
            if (!pendingCourseId) {
            ...
            const response = await fetch(
              `${process.env.NEXT_PUBLIC_API_URL}/api/user-courses/enroll`,
              {
                method: "POST",
                headers: {
  ...
  const handleAddRating = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error?.message || data.error || "Failed to add rating"
        );
      }
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to add rating";
                      ...
                      <div className="p-4 flex items-center justify-between hover:bg-gray-50 transition duration-150">
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              {course.exam.questions.length} questions
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center">
